...
# Fallback: si el DSN principal quedó en localhost y sí tenemos EVENTS_DSN remoto, úsalo
_DEFAULT_LOCAL = "postgresql://postgres:postgres@localhost:5432/postgres"
if DSN == _DEFAULT_LOCAL and EVENTS_DSN and EVENTS_DSN != DSN:
    print("[DB] No DATABASE_URL set; using EVENTS_DSN as main DSN")
    DSN = EVENTS_DSN
...
# ---------- Tenancy ----------
# IMPORTANTE: desactivamos por defecto el set_config de GUCs (rompe en tu PG)
APPLY_RLS_GUCS = os.getenv("DB_SET_RLS_GUCS", "0").lower() in ("1", "true", "yes")
...
def _apply_rls_context(conn: "psycopg.Connection") -> None:
    ...
    # Proteger cada set_config individualmente (si el GUC no existe, lo ignoramos)
    try:
        ...
        # No dejes que el seteo de contexto rompa la request
        pass

# ---------- Parámetros tunables ----------
DB_DISABLE_POOL    = os.getenv("DB_DISABLE_POOL", "0").lower() in ("1", "true", "yes")
DB_POOL_MIN        = int(os.getenv("DB_POOL_MIN", "1"))
DB_POOL_MAX        = int(os.getenv("DB_POOL_MAX", "5"))  # 5 va bien en Supabase free
...
def get_conn():
    ...
    - 1 retry corto ante OperationalError.
    - NO rompe si fallan GUCs: _apply_rls_context está protegido.
    """
