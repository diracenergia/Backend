...
# Permitimos letras, números y separadores típicos de device IDs
...
class TankIngestIn(BaseModel):
    ...
    ts: Optional[datetime] = Field(None, description="Timestamp; si no viene, la DB usa now()")
    # Aceptamos int o str; Pydantic normaliza a str en el Out
...
class TankIngestOut(BaseModel):
    ...
    device_id: Optional[str] = None  # normalizamos a str en el out
