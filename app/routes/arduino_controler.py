...
from psycopg.types.json import Json  # ✅ para serializar dicts a JSON (jsonb)
...
def create_command(cmd: CommandIn):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"DB error (command): {e}")
...
def push_heartbeat(body: HeartbeatIn):
    try:
        with get_conn() as conn, conn.cursor(row_factory=dict_row) as cur:
            ...
            # ✅ payload se serializa a JSON (jsonb) con psycopg
            cur.execute(
                """
                ...
                (
                    body.pump_id,
                    body.rssi,
                    Json(body.payload) if body.payload is not None else None,
                ),
            )
            row = cur.fetchone()
            conn.commit()
        ...
        # Devolver detalle para diagnóstico (temporal)
        raise HTTPException(status_code=500, detail=f"DB error (heartbeat): {e}")
...
def heartbeat_get(pump_id: int = Query(...)):
    try:
        ...
        raise HTTPException(status_code=500, detail=f"DB error (hb): {e}")
...
def push_state(body: StateIn):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"DB error (state): {e}")
...
def state_get(pump_id: int = Query(...), relay: int = Query(...)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"DB error (st): {e}")
...
def next_commands(pump_id: int = Query(...), limit: int = Query(5, ge=1, le=50)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"DB error (next_commands): {e}")
