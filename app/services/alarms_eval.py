...
# Firma recomendada (usa NOTIFY con json.dumps)
...
def _decide_state(level_pct: float, cfg: dict) -> Optional[Tuple[str, str, str]]:
    ...
    cfg requiere: low_low_pct, low_pct, high_pct, high_high_pct
    """
    log.debug(
        "decide_state start level_pct=%.3f cfg_keys=%s", level_pct, list(cfg.keys())
    )
    ...
    if level_pct <= cfg["low_low_pct"]:
        ...
        log.debug("decide_state result code=%s sev=%s key=low_low", code, sev_db)
        return (code, sev_db, "low_low")
    if level_pct <= cfg["low_pct"]:
        code, sev_db = _THRESHOLD_MAP["low"]
        log.debug("decide_state result code=%s sev=%s key=low", code, sev_db)
        return (code, sev_db, "low")
    if level_pct >= cfg["high_high_pct"]:
        code, sev_db = _THRESHOLD_MAP["high_high"]
        log.debug("decide_state result code=%s sev=%s key=high_high", code, sev_db)
        return (code, sev_db, "high_high")
    if level_pct >= cfg["high_pct"]:
        code, sev_db = _THRESHOLD_MAP["high"]
        log.debug("decide_state result code=%s sev=%s key=high", code, sev_db)
        return (code, sev_db, "high")

    log.debug("decide_state result=normal")
    return None  # normal
...
def _clear_one(alarm_id: int, *, asset_type: str, asset_id: int, code: str,
    ...
    """
    log.info(
        "clear_one start alarm_id=%s asset_type=%s asset_id=%s code=%s value=%.3f",
        alarm_id, asset_type, asset_id, code, value
    )
    try:
        cleared = alarms_repo.clear(alarm_id, ts_cleared=_utcnow())
        log.debug("clear_one repo.clear done cleared=%s", bool(cleared))
    except Exception as e:
        log.exception("clear_one repo.clear error err=%s alarm_id=%s", e, alarm_id)
        return

    if not cleared:
        log.info("clear_one skip reason=already_cleared alarm_id=%s", alarm_id)
        return
    ...
    try:
        ...
        log.info(
            "clear_one published op=CLEARED asset_type=%s asset_id=%s code=%s ts=%s",
...
def _clear_all_for_tank(tank_id: int, *, value: float) -> None:
    ...
    try:
        ...
        log.exception("clear_all_for_tank fetch error err=%s tank_id=%s", e, tank_id)
        return
...
def eval_tank_alarm(tank_id: int, level_pct: Optional[float]) -> Optional[int]:
    ...
    try:
        ...
        log.debug(
            "cfg loaded tank_id=%s low_low=%.3f low=%.3f high=%.3f high_high=%.3f",
            tank_id, cfg["low_low_pct"], cfg["low_pct"], cfg["high_pct"], cfg["high_high_pct"]
        )
    except Exception as e:
        log.exception("cfg error err=%s tank_id=%s", e, tank_id)
        return None
    ...
    try:
        ...
        log.exception("decide_state error err=%s tank_id=%s", e, tank_id)
        return None
    ...
    log.info(
        "eval out_of_range code=%s severity=%s alias=%s level=%.3f",
    ...
    try:
        ...
        log.debug("active_lookup done exists=%s", bool(active))
    except Exception as e:
    ...
    try:
        created = alarms_repo.create(
            ...
            ts_raised=_utcnow(),
            is_active=True,
            extra=Json({"value": level_pct, "threshold": threshold_alias}),  # ⚠️ Json(...)
        )
        alarm_id = created.id
        log.info(
            "db_create ok alarm_id=%s tank_id=%s code=%s severity=%s value=%.3f thr=%s",
            alarm_id, tank_id, alarm_code_upper, severity_db_lower, level_pct, threshold_alias
        )
    except Exception as e:
        log.exception(
            "db_create error err=%s tank_id=%s code=%s severity=%s value=%.3f thr=%s",
            e, tank_id, alarm_code_upper, severity_db_lower, level_pct, threshold_alias
        )
        return None
    ...
    try:
        ...
        log.info(
            "publish ok op=RAISED tank_id=%s code=%s severity=%s ts=%s",
            tank_id, alarm_code_upper, severity_db_lower, ts
        )
    except Exception as e:
