...
ENABLED = _truthy(os.getenv("ALARM_POLLER_ENABLED", "0"))  # default OFF

# Si NO está habilitado, exponemos stubs no-op y evitamos importar nada más.
if not ENABLED:
    def start_alarm_poller() -> None:
        # No hace nada cuando está deshabilitado
        return
    ...
    # ====== Implementación real (solo se carga si ENABLED=True) ======
    import time
    import threading
    import logging
    ...
    DEBUG_TG = _truthy(os.getenv("TELEGRAM_DEBUG"))

    # --- Sender de Telegram (opcional) ---
    def _tg_send_urllib(text: str):
        import json
    ...
    try:
        # Si tenés un sender propio disponible
        from app.services.telegram import send as _tg_send_local  # type: ignore
        ...
        tg_send = _tg_send_urllib  # fallback
    ...
    ONLY_ACTIVE = _truthy(os.getenv("ALARM_POLL_ONLY_ACTIVE", "true"))
        ...
        with get_conn() as conn, conn.cursor() as cur:
            ...
            for r in rows:
                ...
                try:
                    ...
                    cur.execute(
                        "update public.alarms set tg_notified_at = now() where id = %s", (a["id"],)
                    )
                    sent += 1
